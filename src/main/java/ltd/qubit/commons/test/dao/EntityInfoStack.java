...
public class EntityInfoStack {
  ...
  public void addReferencedProperty(final Property property) {
    ...
    while (iter.hasNext()) {
      info = iter.next();
      if (!referencePath.startsWith(PARENT_PATH)) {
        break;
      }
      ...
      referencePath = referencePath.substring(PARENT_PATH.length());
      if (referencePath.startsWith(PATH_SEPARATOR)) {
        referencePath = referencePath.substring(PATH_SEPARATOR.length());
      }
      propertyPath = info.getPropertyInParent().getName() + PATH_SEPARATOR + propertyPath;
    }
    if (referencePath.startsWith(PARENT_PATH)) {
  ...
  private Class<?> getReferToEntityType(final Class<?> modelType, final String path) {
    ...
    final int pos = path.indexOf(PATH_SEPARATOR);
